...
#import "RecentLikesViewController.h"


@implementation RecentLikesViewController
...
-(id)initWithFriendUsername:(NSString *)theFriendUsername {
	if ((self = [super initWithNibName:@"PostsViewController" bundle:[NSBundle mainBundle]])) {
		...
		[titleLabel setText:@"Most Recent Likes"];
		...
		[[OperationsManager sharedInstance] setLikesDelegate:self];
	}
	return self;
}
...
-(void)viewWillAppear:(BOOL)animated {
	...
	if (friendUsername) {
		[[OperationsManager sharedInstance] fetchRecentLikes:friendUsername forPage:1];
	} else {
		[[OperationsManager sharedInstance] fetchMyRecentLikes:1];
	}
}
...
-(void)viewWillDisappear:(BOOL)animated {
	...
	[[OperationsManager sharedInstance] setLikesDelegate:nil];
...
-(void)didFetchRecentLikes:(PaginatedLikes *)thePager {
	...
	if ([pager currentPage] > 1) {
		[self setLikes:[likes arrayByAddingObjectsFromArray:[pager likes]]];
	} else {
		[self setLikes:[pager likes]];
	}
...
-(void)loadMore:(id)sender {
	...
	[activityView startAnimating];
	if (friendUsername) {
		[[OperationsManager sharedInstance] fetchRecentLikes:friendUsername forPage:[pager currentPage] + 1];
	} else {
...
-(UITableViewCell *)tableView:(UITableView *)theTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[timestamp setText:[[[like createdAt] date] timeAgo]];
	[username setText:[like username]];
